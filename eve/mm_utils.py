...
def recover_original_image(image, image_mask):
    # Convert image_mask to a numpy array for easier processing
    mask_array = image_mask.squeeze().numpy()  # Remove batch dimension if present

    # Find the bounding box of the area where the mask is 1
    if mask_array.ndim == 2:  # Width > Height case
        rows = torch.any(image_mask == 1, dim=1).nonzero().squeeze()
        cols = torch.any(image_mask == 1, dim=0).nonzero().squeeze()
    else:  # Height > Width case
        rows = torch.any(image_mask == 1, dim=2).nonzero().squeeze()
        cols = torch.any(image_mask == 1, dim=1).nonzero().squeeze()

    # Get bounding box coordinates
    y_min, y_max = rows.min().item(), rows.max().item()
    x_min, x_max = cols.min().item(), cols.max().item()

    # Crop the original image from the larger image
    original_image = image.crop((x_min, y_min, x_max + 1, y_max + 1))

    return original_image

def process_images(images, processor, model_cfg=None, output_raw=False):
    ...
    for raw_image in images:
        ...
        img_token_length = image.size[0] * image.size[1] // (14*14)
        # if output_raw:
        ...
        if output_raw:
            ...
            new_raw_images.append(image_clip)
            image_token_length.append(img_token_length)
    ...
    if output_raw:
        return new_images, new_raw_images, image_token_length
    else:
        return new_images


def tokenizer_image_token(prompt, tokenizer, image_token_index=IMAGE_TOKEN_INDEX, return_tensors=None):
    prompt_chunks_temp = prompt.split('<image>')
    prompt_chunks = []
    for chunk in prompt_chunks_temp:
        if "&&&&" in chunk:
            chunk_list = chunk.split('&&&&')
            chunk_ids = tokenizer(chunk_list[0]).input_ids
            chunk_ids.extend([-300])
            chunk_ids.extend(tokenizer(chunk_list[1]).input_ids)
